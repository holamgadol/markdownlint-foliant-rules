...
const { loadYAML, readApiRefRegistryDir } = require('./apireferences-check-tools/load-configs')
...
module.exports = {
  ...
  function: (params, onError) => {
    ...
    const apirefregistry = readApiRefRegistryDir(apirefregistryPath)
    ...
    const regexArr = [
      ...
      new RegExp(`\\s*((?<prefix>[\\w-]+):\\s+)((?<verb>${verbs})\\s+)(?<command>[^\`\\n]+?)\\s*`),
      new RegExp(`\\s*((?<verb>${verbs})\\s+)(?<command>[^\`\\n]+?)\\s*`),
      new RegExp(`\\s*((?<prefix>[\\w-]+):\\s+)((?<verb>${verbs})\\s+)?(?<command>(?!.*[\`\\n]).*/.*?)\\s*`)
    ]
        ...
        if (prefix === undefined) {
          ...
          if (Object.hasOwn(apirefregistry, prefix)) {
            ...
            if (Array.isArray(apirefregistry[prefix])) {
              if (!refs.some(el => apirefregistry[prefix].includes(toIdSlate(el)))) {

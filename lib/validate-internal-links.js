...
module.exports = {
  ...
  function: function validateInternalLinks (params, onError) {
    ...
    let includesMap = params.config.includesMap || './includes_map.json'
    if (typeof includesMap === 'string') {
      const includesMapPath = path.resolve(workingDir, includesMap)
      if (fs.existsSync(includesMapPath)) {
        includesMap = require(includesMapPath)
      }
    }
            ...
            if (fs.existsSync(file)) {
              if (anchor) {
                ...
                if (includesMap.length > 0) {
                  findAnchorInIncludes(workingDir, file, anchor, refs, includesMap)
                }
                if (!refs.has(link)) {
